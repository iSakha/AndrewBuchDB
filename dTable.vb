...
Module dTable
    ...
    '                === Format DataGridView ===
    '===================================================================================
    Sub DGV_format(_color As Color)
        Dim i As Integer = mainForm.tabControl.SelectedIndex
        Select Case i

            Case > 0
                mainForm.dgv(i - 1).Columns(0).Width = 40                ' #
                mainForm.dgv(i - 1).Columns(1).Width = 175               ' Fixture
                mainForm.dgv(i - 1).Columns(2).Width = 40                ' Q-ty
                mainForm.dgv(i - 1).Columns(2).DefaultCellStyle.Alignment = DataGridViewContentAlignment.MiddleCenter
                mainForm.dgv(i - 1).Columns(3).Width = 220               ' BelImlight_1  (PRLightigTouring, BlackOut, Vision, Stage)
                mainForm.dgv(i - 1).Columns(4).Width = 40                ' Q-ty_1
                mainForm.dgv(i - 1).Columns(4).DefaultCellStyle.Alignment = DataGridViewContentAlignment.MiddleCenter
                mainForm.dgv(i - 1).Columns(5).Width = 220               ' BelImlight_2  (PRLightigTouring, BlackOut, Vision, Stage)
                mainForm.dgv(i - 1).Columns(6).Width = 40                ' Q-ty_2
                mainForm.dgv(i - 1).Columns(6).DefaultCellStyle.Alignment = DataGridViewContentAlignment.MiddleCenter
                mainForm.dgv(i - 1).Columns(7).Width = 180               ' BelImlight_3  (PRLightigTouring, BlackOut, Vision, Stage)
                mainForm.dgv(i - 1).Columns(8).Width = 40                ' Q-ty_3
                mainForm.dgv(i - 1).Columns(8).DefaultCellStyle.Alignment = DataGridViewContentAlignment.MiddleCenter

                For j As Integer = 0 To mainForm.dgv(i - 1).Rows.Count - 2

                    'mainForm.DGV_in.Rows(i).Cells(1).Value = Date.FromOADate(mainForm.DGV_in.Rows(i).Cells(1).Value)
                    mainForm.dgv(i - 1).RowsDefaultCellStyle.BackColor = _color
                    mainForm.dgv(i - 1).AlternatingRowsDefaultCellStyle.BackColor = Color.FromArgb(250, 250, 250)
    ...
    Sub dgv_clickCell(_sender As Object, _e As DataGridViewCellEventArgs)
        ...
        Try
            ...
            mainForm.dgv(i - 1).Rows(index).Selected = True
            sumForm.dgv_sum.ClearSelection()
            sumForm.dgv_sum.Rows(index).Selected = True

            If index = mainForm.dgv(i - 1).Rows.Count - 1 Then
                writeZeroInQtyTxt()
            End If
        Catch
